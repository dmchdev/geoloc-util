...
@pytest.mark.asyncio
async def test_single_word_city_state():
...
@pytest.mark.asyncio
async def test_multiple_word_city_state():
    schema = schemas.city_state_response_schema
...
@pytest.mark.asyncio
async def test_missing_comma_city_state():
    schema = schemas.city_state_response_schema
    report = geoloc.process_locations(["Wilmette IL"])
    validate(instance=report, schema=schema)
    assert(report[0]["name"] == "Wilmette")
...
@pytest.mark.asyncio
async def test_missing_comma_multiple_word_city_state():
    schema = schemas.city_state_response_schema
    report = geoloc.process_locations(["Los Angeles CA"])
    validate(instance=report, schema=schema)
    assert(report[0]["name"] == "Los Angeles")
...
@pytest.mark.asyncio
async def test_zip_with_letters():
    report = geoloc.process_locations(["605b1"])
    assert(report[0] == [])

@pytest.mark.asyncio
async def test_nonexistent_city():
...
@pytest.mark.asyncio
async def test_incorrect_city_state_combination():
